...
#include "..\STM32L4_Power\STM32L4_Power_functions.h"

#include "STM32L4_ETH_lwip_adapter.h"
#include "STM32L4_ETH_lwip.h"
#include "STM32L4_ETH_driver.h"

static struct netif          g_STM32L4_ETH_NetIF;
...
err_t STM32L4_ETH_ethhw_init( struct netif *myNetIf )
{
    /* Initialize netif */
    myNetIf->name[ 0 ] = 'e';
    myNetIf->name[ 1 ] = '0';
...
void lwip_interrupt_continuation( void )
{
    ...
    STM32L4_ETH_LWIP_recv( &g_STM32L4_ETH_NetIF );
}
...
void EthernetWakeUp( )
{
    CPU_INTC_ActivateInterrupt( ETH_IRQn, ( HAL_CALLBACK_FPN )STM32L4_ETH_LWIP_interrupt, &g_STM32L4_ETH_NetIF );
    InitContinuations( &g_STM32L4_ETH_NetIF );
}

void EthernetDeepSleep( )
{
    netifapi_netif_set_down( &g_STM32L4_ETH_NetIF );
    STM32L4_ETH_LWIP_close( FALSE );
    LinkStatus = 0;
    eth_powerUpPhy( FALSE );
...
int STM32L4_ETH_LWIP_Driver::Open( int index )
{
    ...
    STM32L4_SetPowerHandlers( EthernetDeepSleep, EthernetWakeUp );

    /* Apply network configuration */
    iface = &g_NetworkConfig.NetworkInterfaces[ index ];

    len = g_STM32L4_ETH_NetIF.hwaddr_len;
    if( len == 0 || iface->macAddressLen < len )
    {
        len = iface->macAddressLen;
        g_STM32L4_ETH_NetIF.hwaddr_len = len;
    }
    memcpy( g_STM32L4_ETH_NetIF.hwaddr, iface->macAddressBuffer, len );

    ipaddr.addr = iface->ipaddr;
    gateway.addr = iface->gateway;
    subnetmask.addr = iface->subnetmask;

    pNetIF = netif_add( &g_STM32L4_ETH_NetIF, &ipaddr, &subnetmask, &gateway, NULL, STM32L4_ETH_ethhw_init, tcpip_input );
    ...
    return g_STM32L4_ETH_NetIF.num;
}

BOOL STM32L4_ETH_LWIP_Driver::Close( void )
{
    CPU_INTC_DeactivateInterrupt( ETH_IRQn );
    ...
    netifapi_netif_set_down( &g_STM32L4_ETH_NetIF );
    netifapi_netif_remove( &g_STM32L4_ETH_NetIF );

    STM32L4_ETH_LWIP_close( TRUE );

    memset( &g_STM32L4_ETH_NetIF, 0, sizeof g_STM32L4_ETH_NetIF );

    return TRUE;
}

BOOL STM32L4_ETH_LWIP_Driver::Bind( void )
{
    return TRUE;
}

BOOL STM32L4_ETH_LWIP_Driver::IsNetworkConnected( void )
{
    return LinkStatus;
}
