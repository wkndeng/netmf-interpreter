...
void STM32L4_I2C_Abort(void)
{
    currentI2CXAction->Signal(I2C_HAL_XACTION::c_Status_Aborted);
}

void STM32L4_I2C_WriteToSubordinate(I2C_HAL_XACTION_UNIT *unit)
{
    UINT8 *queueData;
...
void STM32L4_I2C_ReadFromSubordinate(I2C_HAL_XACTION_UNIT *unit)
{
    UINT8* queueData;
    ...
    if (todo == 0) {
    	if (ISR & I2C_ISR_TC) {
    		if (!xAction->ProcessingLastUnit()) {
    			...
    			I2Cx->CR2 = (I2Cx->CR2 & ~0x000003ff) | (UINT32)(xAction->m_address << 1);
    			I2Cx->CR2 = (I2Cx->CR2 & ~0x00ff0000) | (UINT32)(currentI2CUnit->m_bytesToTransfer);
    			I2Cx->CR1 = I2C_INT_FLAGS + I2C_CR1_PE; 	// enable and reset special flag
    			I2Cx->CR2 |= I2C_CR2_START;	// send start
    		} else {
...
//            ((UINT32)I2C_RELOAD_MODE) |
        ...
        I2Cx->CR1 = I2C_INT_FLAGS + I2C_CR1_PE;     // enable and reset special flag
        while (currentI2CUnit->m_bytesToTransfer > 0)
            STM32L4_I2C_ReadFromSubordinate(currentI2CUnit);
    } else {
        I2Cx->CR2 = (I2Cx->CR2 & ~(CR2_MASK)) |
            ((UINT32)(xAction->m_address) << 1) |
            ((UINT32)(currentI2CUnit->m_bytesToTransfer) << 16) |
...
//            ((UINT32)I2C_RELOAD_MODE) |
        ...
        I2Cx->CR1 = I2C_INT_FLAGS + I2C_CR1_PE;     // enable and reset special flag
        while (currentI2CUnit->m_bytesToTransfer > 0)
            STM32L4_I2C_WriteToSubordinate(currentI2CUnit);
    }
}
