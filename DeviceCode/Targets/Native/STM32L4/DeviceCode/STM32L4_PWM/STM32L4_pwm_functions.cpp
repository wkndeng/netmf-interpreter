...
//Timers
static const BYTE g_STM32L4_PWM_Timer[] = STM32L4_PWM_TIMER;
static const BYTE g_STM32L4_PWM_Channel[] = STM32L4_PWM_CHNL;

// Pins
static const BYTE g_STM32L4_PWM_Pins[] = STM32L4_PWM_PINS;
#define STM32L4_PWM_CHANNELS ARRAYSIZE_CONST_EXPR(g_STM32L4_PWM_Pins) // number of channels

// IO addresses
static const ptr_TIM_TypeDef g_STM32L4_PWM_Ports[] =
    {TIM1, TIM2, TIM3, TIM4, TIM5, NULL, NULL, TIM8};
    
// Alternate Function Number
static const BYTE g_STM32L4_PWM_Alt[] = {0x12,0x12,0x22,0x22,0x22,0,0,0x32,0x32,0x32,0x32,0x92,0x92,0x92}; // AF1/2/3/9


//--//

void STM32L4_PWM_UnitializeAll()
{
    for (int c = 0; c < STM32L4_PWM_CHANNELS; c++) {
...
{
    if (channel >= STM32L4_PWM_CHANNELS) return FALSE;
    int timer = g_STM32L4_PWM_Timer[channel];
    int tchnl = g_STM32L4_PWM_Channel[channel];
    ptr_TIM_TypeDef treg = g_STM32L4_PWM_Ports[timer - 1];
    ...
    *reg |= mode;
    ...
    HAL_AddSoftRebootHandler(STM32L4_PWM_UnitializeAll);
#endif
    return TRUE;
}
...
#if 0
    CPU_GPIO_DisablePin( pin, RESISTOR_DISABLED, 1, (GPIO_ALT_MODE)g_STM32L4_PWM_Alt[timer - 1] );
...
{
    return STM32L4_PWM_CHANNELS;
}

GPIO_PIN PWM_GetPinForChannel( PWM_CHANNEL channel )
{
    if ((UINT32)channel >= STM32L4_PWM_CHANNELS) return GPIO_PIN_NONE;
    return g_STM32L4_PWM_Pins[channel];
}
