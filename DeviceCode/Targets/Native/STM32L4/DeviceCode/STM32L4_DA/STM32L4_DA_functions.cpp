...
#define STM32L4_DA_CHANNELS 2       // number of channels
#define STM32L4_DA_FIRST_PIN 4      // channel 0 pin (A4)
...
BOOL DA_Initialize( DA_CHANNEL channel, INT32 precisionInBits )
{
    ...
    // set pin as analog
    CPU_GPIO_DisablePin(STM32L4_DA_FIRST_PIN + channel, RESISTOR_DISABLED, 0, GPIO_ALT_MODE_1);
    if (channel) {
...
void DA_Uninitialize( DA_CHANNEL channel )
{
    ...
    // free pin
    CPU_GPIO_DisablePin(STM32L4_DA_FIRST_PIN + channel, RESISTOR_DISABLED, 0, GPIO_ALT_PRIMARY);
...
{
    return STM32L4_DA_CHANNELS;
}

GPIO_PIN DA_GetPinForChannel( DA_CHANNEL channel )
{
    if ((UINT32)channel >= STM32L4_DA_CHANNELS) return GPIO_PIN_NONE;
    return STM32L4_DA_FIRST_PIN + channel;
}

BOOL DA_GetAvailablePrecisionsForChannel( DA_CHANNEL channel, INT32* precisions, UINT32& size )
{
    size = 0;
    if (precisions == NULL || (UINT32)channel >= STM32L4_DA_CHANNELS) return FALSE;
    precisions[0] = 12;
    size = 1;
    return TRUE;
}
